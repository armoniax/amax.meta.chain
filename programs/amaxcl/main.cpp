...
string default_wallet_url = "unix://" + (determine_home_directory() / wallet_folder_name / (string(key_store_executable_name) + ".sock")).string();
...
void ensure_amaxks_running(CLI::App* app) {
    ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if amaxcl and amaxks are in the same installation directory
