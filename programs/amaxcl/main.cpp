/**
  ...
  @section intro Introduction to amaxcl

  `amaxcl` is a command line tool that interfaces with the REST api exposed by @ref amaxnd. In order to use `amaxcl` you will need to
...
Options:
  ...
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where amaxkm is running
...
//copy pasta from amaxkm's main.cpp
...
inline asset to_asset( const string& s ) {
   return to_asset( N(amax.token), s );
}
...
void ensure_amaxkm_running(CLI::App* app) {
    if (no_auto_amaxkm)
        return;
    // get, version, net, convert do not require amaxkm
    ...
    // This extra check is necessary when running amaxcl like this: ./amaxcl ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if amaxcl and amaxkm are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("amaxkm").append(key_store_executable_name);
    }
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Change to voter.value when amaxcl no longer needs to support amaxnd versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when amaxcl no longer needs to support amaxnd versions older than 1.5.0
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so amaxcl can still work with old buggy amaxnd versions
                               // Change to newname.value when amaxcl no longer needs to support amaxnd versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
   ...
   if (!json_format) {
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of amaxnd responding to the request is different than core symbol built into amaxcl
...
int main( int argc, char** argv ) {
   ...
   app.add_flag( "--no-auto-" + string(key_store_executable_name), no_auto_amaxkm, localized("don't automatically launch a ${k} if one is not currently running", ("k", key_store_executable_name)));
   app.parse_complete_callback([&app]{ ensure_amaxkm_running(&app);});
...
//                                ->check(CLI::ExistingFile);
   ...
   stopKeosd->callback([] {
      const auto& v = call(wallet_url, amaxkm_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success amaxkm responds with empty object
   ...
   propose_action->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(amax.msig), N(propose), variant_to_bin( N(amax.msig), N(propose), args ) }});
   });
   ...
   review->callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so amaxcl can still work with old buggy amaxnd versions
                                 // Change to name(proposal_name).value when amaxcl no longer needs to support amaxnd versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when amaxcl no longer needs to support amaxnd versions older than 1.5.0
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(amax.msig), name(action), variant_to_bin( N(amax.msig), name(action), args ) }});
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(amax.msig), N(invalidate), variant_to_bin( N(amax.msig), N(invalidate), args ) }});
   });
   ...
   cancel->callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, N(amax.msig), N(cancel), variant_to_bin( N(amax.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(amax.msig), N(exec), variant_to_bin( N(amax.msig), N(exec), args ) }});
      }
   );
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send amax.system contract action to the blockchain."));
