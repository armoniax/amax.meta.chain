...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.amcli + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def stepCreateTokens():
    run(args.amcli + 'push action amax.token create \'["amax", "10000000000.0000 %s"]\' -p amax.token' % (args.symbol))
...
commands = [
    ('k', 'kill',               stepKillAll,                True,    "Kill all amnod and amkey processes"),
...
parser.add_argument('--symbol', metavar='', help="The amax.system symbol", default='SYS')
