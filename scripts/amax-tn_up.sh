#!/bin/bash
#
# amax-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# amax-tn_bounce.sh and amax-tn_roll.sh scripts.
...
if [ "$PWD" != "$AMAX_HOME" ]; then
    echo $0 must only be run from $AMAX_HOME
    exit -1
fi
...
datadir=var/lib/node_$AMAX_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/amax/node_$AMAX_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/amax/node_$AMAX_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
