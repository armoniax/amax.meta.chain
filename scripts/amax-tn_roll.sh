...
# the command line.
#
# In most cases, simply running ./amax-tn_roll.sh is sufficient.
#

if [ -z "$AMAX_HOME" ]; then
    echo AMAX_HOME not set - $0 unable to proceed.
    exit -1
fi
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for eosd or amaxd or amaxnd
    exit 1
fi
...
if [ $DD = "all" ]; then
    for AMAX_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $AMAX_HOME/scripts/amax-tn_up.sh "$*"
    done
else
    bash $AMAX_HOME/scripts/amax-tn_up.sh "$*"
fi
unset AMAX_RESTART_DATA_DIR
