...
void test_action::test_cf_action() {
   ...
   if ( cfa.payload == 100 ) {
      ...
      send_deferred( "testapi"_n.value, "testapi"_n, "hello", 6, false );
      check( false, "transaction_api should not be allowed" );     
   }
...
void test_action::require_auth() {
   eosio::internal_use_do_not_use::prints("require_auth");
...
void test_action::test_publication_time() {
   ...
   check( pub_time == publication_time().sec_since_epoch(), "pub_time == publication_time()" );
}
...
void test_action::test_current_time() {
   ...
   check( total == sizeof(uint64_t), "total == sizeof(uint64_t)" );
   check( tmp == current_time_point().sec_since_epoch(), "tmp == current_time()" );
}
