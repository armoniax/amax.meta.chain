...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(amax.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(amax.token), contracts::amax_token_wasm() );
         chain->set_abi(N(amax.token), contracts::amax_token_abi().data() );
         chain->push_action( N(amax.token), N(create), N(amax.token), mvo()
              ( "issuer", "amax.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(amax.token), N(issue), N(amax.token), mvo()
              ( "to", "amax.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(amax.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(amax.token), N(alice)};
   test.init();

   test.transfer( N(amax.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(amax.token), N(bob)};
   test.init();

   test.transfer( N(amax.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(amax.token) );
   ...
   test.chain->set_code(N(bob), contracts::amax_token_wasm() );
   test.chain->set_abi(N(bob), contracts::amax_token_abi().data() );

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), contracts::amax_token_wasm() );
   test.chain->set_abi(N(charlie), contracts::amax_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(amax.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_eosio ) { try {
   ...
   tester1.chain->set_code(config::system_account_name, contracts::amax_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   tester1.chain->push_action( N(amax), N(updateauth), N(alice), mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( N(amax), N(updateauth), N(bob), mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( N(amax), N(updateauth), N(charlie), mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const signed_transaction&> x) {
      ...
      if( act.account == N(amax) && act.name == N(onblock) ) return;
