...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for amax reqauth action with alice's spending key
   chain.link_authority(name("alice"), name("amax"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("amax"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with amax reqauth
   chain.unlink_authority(name("alice"), name("amax"), name("reqauth"));
   ...
   // Link authority for any amax action with alice's scud key
   chain.link_authority(name("alice"), name("amax"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("amax"), name("first"), name("reqauth"));
...
try {
   ...
   // Creating account with amax. prefix with privileged account
   chain.create_account(name("amax.test1"));

   // Creating account with amax. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("amax.test2"), name("joe")), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'amax.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("amax"), name("amax.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("amax"), name("amax.any"), name("reqauth") );
