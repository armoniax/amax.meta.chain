...
class amax_token_tester : public tester {
public:
   ...
   fc::variant get_stats( const string& symbolname )
   {
      ...
      vector<char> data = get_row_by_account( N(amax.token), name(symbol_code), N(stat), name(symbol_code) );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(amax.token), acc, N(accounts), name(symbol_code) );
...
BOOST_AUTO_TEST_SUITE(amax_token_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, amax_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, amax_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, amax_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, amax_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, amax_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, amax_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, amax_token_tester ) try {
