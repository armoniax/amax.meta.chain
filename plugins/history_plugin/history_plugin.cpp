...
namespace eosio {
   ...
   class history_plugin_impl {
      public:
         ...
         void apply_before_incoming_transaction() {
            if ( producer_plug ) {  // producer_plug is null when called from chain_plugin_tests.cpp and get_table_tests.cpp
               producer_plug->get_before_incoming_transaction().connect([self=this](const packed_transaction_ptr& trx) {
                  auto& chain = self->chain_plug->chain();
   ...
   void history_plugin::plugin_initialize(const variables_map& options) {
      try {
         ...
         my->apply_before_incoming_transaction();
         ...
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               if( s == "*" || s == "\"*\"" ) {
                  ...
                  wlog( "--filter-on * enabled. This can fill shared_mem, causing amnod to stop." );
                  break;
               }
