  ...
  FC_MULTILINE_MACRO_END
   ...
   void connection::process_signed_block( const block_id_type& blk_id, signed_block_ptr msg ) {
      ...
      try {
         ...
         ilog("accepted block has been processed completly by on_incoming_block()........."); // TODO: remove?
   ...
   void net_plugin_impl::on_accepted_block(const block_state_ptr& bs) {
      ilog("on accepted block trigered by emit signal...."); // TODO: remove?
      update_chain_info();
      controller& cc = chain_plug->chain();
   ...
   void net_plugin_impl::on_pre_accepted_block(const signed_block_ptr& block) {
      ilog("on pre accepted block trigered by emit signal...."); // TODO: remove?
      update_chain_info();
      controller& cc = chain_plug->chain();
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"AMAX Test Agent\""), "The name supplied to identify this node amongst the peers.")
