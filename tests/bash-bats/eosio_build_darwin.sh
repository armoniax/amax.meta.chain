...
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting AMAX Dependency Install") ]] || exit
    ...
    # TODO: brew tap amax/amax
    # [[ ! -z $(echo "${output}" | grep "brew tap amax/amax") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting AMAX Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "AMAX has been successfully built") ]] || exit
}