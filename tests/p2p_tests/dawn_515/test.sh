...
read -d '' genesis << EOF
{
  ...
  "initial_key": "AMA6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
...
private-key = ['AMA6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV','5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3']
...
programs/amax-launcher/amax-launcher -p $pnodes -n $total_nodes --nogen -d $delay
...
b5idbios=`./programs/amaxcl/amaxcl -u http://localhost:8888 get block 5 | grep "^ *\"id\""`
b5id00=`./programs/amaxcl/amaxcl -u http://localhost:8889 get block 5 | grep "^ *\"id\""`
b5id01=`./programs/amaxcl/amaxcl -u http://localhost:8890 get block 5 | grep "^ *\"id\""`
...
programs/amax-launcher/amax-launcher -k 15
rm -rf staging
rm -rf var/lib/node_*
rm -rf etc/amax/node_*
exit $ret
