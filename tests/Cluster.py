...
class Cluster(object):
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraAmnodArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraAmnodArgs=None, onlySetProds=False,
        ...
        extraAmnodArgs: string of arguments to pass through to each nodoes instance (via --amnod flag on launcher)
        ...
        specificExtraAmnodArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-amnod flags on launcher), example: { "5" : "--plugin eosio::test_control_api_plugin" }
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on amnod default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        amnodArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            amnodArgs += " --plugin eosio::wallet_api_plugin"
        if self.enableMongo:
            amnodArgs += " --plugin eosio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraAmnodArgs is not None:
            assert(isinstance(extraAmnodArgs, str))
            amnodArgs += extraAmnodArgs
        if Utils.Debug:
            amnodArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            amnodArgs += " --plugin eosio::producer_api_plugin"

        if amnodArgs:
            cmdArr.append("--amnod")
            cmdArr.append(amnodArgs)

        if specificExtraAmnodArgs is not None:
            assert(isinstance(specificExtraAmnodArgs, dict))
            for nodeNum,arg in specificExtraAmnodArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-amnod")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-amnod")
                cmdArr.append(path)
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster amnod instances. allInstances will kill all amnod instances running on the system."""
    ...
    def bounce(self, nodes, silent=True):
        """Bounces amnod instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down amnod instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
