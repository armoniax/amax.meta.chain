...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
        ...
        walletHost: ama wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures ama mongo plugin
        ...
        self.amaxAccount=self.defProducerAccounts["amax"]= Account("amax")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraAmaxndArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraAmaxndArgs=None, onlySetProds=False,
        ...
        extraAmaxndArgs: string of arguments to pass through to each nodoes instance (via --amaxnd flag on launcher)
        ...
        specificExtraAmaxndArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-amaxnd flags on launcher), example: { "5" : "--plugin eosio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the amax.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on amaxnd default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        amaxndArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            amaxndArgs += " --plugin eosio::wallet_api_plugin"
        if self.enableMongo:
            amaxndArgs += " --plugin eosio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraAmaxndArgs is not None:
            assert(isinstance(extraAmaxndArgs, str))
            amaxndArgs += extraAmaxndArgs
        if Utils.Debug:
            amaxndArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            amaxndArgs += " --plugin eosio::producer_api_plugin"

        if amaxndArgs:
            cmdArr.append("--amaxnd")
            cmdArr.append(amaxndArgs)

        if specificExtraAmaxndArgs is not None:
            assert(isinstance(specificExtraAmaxndArgs, dict))
            for nodeNum,arg in specificExtraAmaxndArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-amaxnd")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-amaxnd")
                cmdArr.append(path)
        ...
        self.amaxAccount=self.defProducerAccounts["amax"]
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.AmaxClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on ama server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on ama node port: %d" % (node.port))
                return False
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.amaxAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse amax private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by amax-launcher."""
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down ama cluster.")
            return None
        ...
        amaxName="amax"
        amaxKeys=producerKeys[amaxName]
        amaxAccount=Account(amaxName)
        amaxAccount.ownerPrivateKey=amaxKeys["private"]
        amaxAccount.ownerPublicKey=amaxKeys["public"]
        amaxAccount.activePrivateKey=amaxKeys["private"]
        amaxAccount.activePublicKey=amaxKeys["public"]
        producerKeys.pop(amaxName)

        if not self.walletMgr.importKey(amaxAccount, ignWallet):
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"amax\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init amax transfer")
            opts="--permission amax@active"
            if name != "amax":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from amax.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(amaxAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, amaxAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission amax@active"
                    myTrans=biosNode.pushMessage("amax", "setprods", setProdsStr, opts)
            ...
            # wait for block production handover (essentially a block produced by anyone but amax).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "amax"
        ...
        amaxTokenAccount=copy.deepcopy(amaxAccount)
        amaxTokenAccount.name="amax.token"
        trans=biosNode.createAccount(amaxTokenAccount, amaxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (amaxTokenAccount.name))
            return None

        amaxRamAccount=copy.deepcopy(amaxAccount)
        amaxRamAccount.name="amax.ram"
        trans=biosNode.createAccount(amaxRamAccount, amaxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (amaxRamAccount.name))
            return None

        amaxRamfeeAccount=copy.deepcopy(amaxAccount)
        amaxRamfeeAccount.name="amax.ramfee"
        trans=biosNode.createAccount(amaxRamfeeAccount, amaxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (amaxRamfeeAccount.name))
            return None

        amaxStakeAccount=copy.deepcopy(amaxAccount)
        amaxStakeAccount.name="amax.stake"
        trans=biosNode.createAccount(amaxStakeAccount, amaxAccount, 0)
        if trans is None:
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(amaxTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=amaxTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (amaxAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to amax contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (amaxAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (amaxAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to amax contract.")
            return None
        ...
        Utils.Print("Verify amax issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountEosBalanceStr(amaxAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (amaxAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (amaxAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (amaxTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (amaxAccount.name)
            trans=biosNode.pushMessage(amaxAccount.name, action, data, opts)
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster amaxnd instances. allInstances will kill all amaxnd instances running on the system."""
        ...
        if allInstances:
            # ocassionally the launcher cannot kill the ama server
            cmd="pkill -9 %s" % (Utils.EosServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down ama cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces amaxnd instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down amaxnd instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
