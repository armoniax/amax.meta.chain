...
namespace eosio { namespace chain {  namespace producer_change_merger {
   ...
   inline void validate(const proposed_producer_changes& changes, const vector<producer_authority>& main_producers, const flat_map<name, block_signing_authority>& backup_producers)
   {
      ...
      {
         const auto& producer_name = change.first;
         bool found = (!main_changes.clear_existed) && exist_in_main_producers(producer_name);
         auto op = (producer_change_operation)change.second.which();

         switch(op) {
            case producer_change_operation::add: {
               main_producer_count++;
               EOS_ASSERT( !found, producer_schedule_exception, "the added main producer:${p} already exists", ("p", producer_name ) );
            ...
            case producer_change_operation::modify:
               ...
               EOS_ASSERT( found, producer_schedule_exception, "the modified main producer:${p} not found", ("p", producer_name ) );
               break;
            case producer_change_operation::del:
               ...
               EOS_ASSERT( found, producer_schedule_exception, "the deleted main producer:${p} not found", ("p", producer_name ) );
      ...
      {
         const auto& producer_name = change.first;
         bool found = (!backup_changes.clear_existed) && ( backup_producers.find(producer_name) != backup_producers.end()) ;
         auto op = (producer_change_operation)change.second.which();
         ...
         switch(op) {
            ...
            case producer_change_operation::modify:
               ...
               EOS_ASSERT( found, producer_schedule_exception, "the modified backup producer:${p} not found", ("p", producer_name ) );
               break;

            case producer_change_operation::del:
               EOS_ASSERT( !backup_changes.clear_existed, producer_schedule_exception, "can not delete backup producer:${p} when clear_existed is true", ("p", producer_name ) );
               if (found) {
