...
const static name system_account_name    { N(amax) };
const static name null_account_name      { N(amax.null) };
const static name producers_account_name { N(amax.prods) };
...
const static name eosio_auth_scope       { N(amax.auth) };
const static name eosio_all_scope        { N(amax.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name eosio_any_name  { N(amax.any) };
const static name eosio_code_name { N(amax.code) };
...
const static uint32_t   default_max_block_cpu_usage                  = 800'000; /// max block cpu usage in microseconds, 80% of block interval time
